...
-export([start/2, start/3, stop/1, start_link/3]).
...
-record(state, {key, callback}).

start(Name, Key) ->
    start(Name, Key, undefined).

start(Name, Key, ResultCallback) ->
    fcm_sup:start_child(Name, Key, ResultCallback).
...
%% OTP
start_link(Name, Key, ResultCallback) ->
    gen_server:start_link({local, Name}, ?MODULE, [Key, ResultCallback], []).

init([Key, ResultCallback]) ->
...
handle_cast({send, RegIds, Message, Retry}, #state{key=Key, callback=CB} = State) ->
    Reply = do_push(RegIds, Message, Key, Retry),
    if
        CB =/= undefined ->
            try CB(Reply) of
